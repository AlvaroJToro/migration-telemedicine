...
const [qaOrders] = await qaPool.query(`
  ...
  for (const item of qaOrders) {
    ...
    const order = {
      ...
      status_code: item.status_code,
      questionnaire_status_code: item.questionnaire_status_code,
      filled_at: item.filled_at ? new Date(item.filled_at) : null,
      patient_id: patient.id,
      shipping_address_id: patient.addressData.id,
      billing_address_id: patient.addressData.id
    }
  ...
  const result1 = await localPool.query(
    `INSERT INTO orders (id, order_type, subscription_type, order_number, skip_verification, 
      reminder, is_nmi, is_ehr, created_at, updated_at, platform_order_id,  
      status_code, questionnaire_status_code, questionnaire_fullfiled_at, patient_id,
      shipping_address_id, billing_address_id, document_url
    ) VALUES ?`,
    [orders.splice(0, 46570)]
  );
  ...
  //   [prescriptions]
  // );
  console.log("ORDERS AND VARIANTS INSERTED");
